...
// --- Response filters ---
// Keep only the fields the companion needs to avoid blowing up the context window.
...
// Interval fields useful for coaching analysis — strips ~60 null fields per interval
const INTERVAL_FIELDS = [
  "type", "label", "distance", "moving_time", "elapsed_time",
  "average_speed", "gap", "average_heartrate", "max_heartrate",
  "average_cadence", "total_elevation_gain", "average_gradient",
  "zone", "intensity",
];

const GROUP_FIELDS = [
  "id", "count", "distance", "moving_time",
  "average_speed", "gap", "average_heartrate",
  "average_cadence", "total_elevation_gain", "zone",
];

function pick<T extends Record<string, unknown>>(obj: T, fields: string[]): Partial<T> {
  const result: Record<string, unknown> = {};
  for (const f of fields) {
    if (f in obj && obj[f] !== null && obj[f] !== undefined) result[f] = obj[f];
  }
...
server.tool(
  ...
  async ({ id }) => {
    try {
      const raw = await fetchAPI(`/activity/${id}?intervals=true`) as Record<string, unknown>;
      const activity = pick(raw, ACTIVITY_FIELDS);

      // Filter intervals to coaching-relevant fields only (~14 fields instead of ~70)
      if (Array.isArray(raw.icu_intervals)) {
        (activity as Record<string, unknown>).icu_intervals = raw.icu_intervals.map(
          (i: Record<string, unknown>) => pick(i, INTERVAL_FIELDS)
        );
      }
      if (Array.isArray(raw.icu_groups)) {
        (activity as Record<string, unknown>).icu_groups = raw.icu_groups.map(
          (g: Record<string, unknown>) => pick(g, GROUP_FIELDS)
        );
      }
      // Pass through zone arrays as-is (small)
      for (const f of ["icu_hr_zones", "icu_pace_zones", "icu_power_zones"]) {
        if (raw[f]) (activity as Record<string, unknown>)[f] = raw[f];
      }
